...
document.addEventListener('DOMContentLoaded', function() {
  // Check if we need to open auth modal
  const openAuthModal = document.body.dataset.openAuthModal;
  if (openAuthModal === 'true') {
    openAuthModal();
  }
  
  // Show error message if present
  const errorMsg = document.body.dataset.errorMessage;
  const errorAction = document.body.dataset.errorAction;
  
  if (errorMsg) {
    showMessage(errorMsg, 'error');
    
    // Show action button based on error type
    if (errorAction === 'wallet') {
      showActionButton('Go to My Account', '/my-account');
    } else if (errorAction === 'group') {
      showActionButton('Find Groups', '/groups');
    } else if (errorAction === 'payment') {
      showActionButton('View Group', '/my-account');
    }
  }
});

function showActionButton(text, url) {
